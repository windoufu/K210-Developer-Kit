...
int main()
{
    ...
    //Store the address of the AI image for algorithm processing by the AI module (red, green, blue/component address)
    dvp_set_ai_addr((uint32_t)kpu_image.addr, (uint32_t)(kpu_image.addr + 320 * 240), (uint32_t)(kpu_image.addr + 320 * 240 * 2));
    //Set the storage address of the captured image in the memory, which can be used to display
    dvp_set_display_addr((uint32_t)display_image.addr);
    //Image start acquisition interrupted | Image end acquisition interrupted
    dvp_config_interrupt(DVP_CFG_START_INT_ENABLE | DVP_CFG_FINISH_INT_ENABLE, 0);
    //Disable automatic image receiving mode
    dvp_disable_auto();
    ...
    plic_set_priority(IRQN_DVP_INTERRUPT, 1);               //Set interrupt priority
    plic_irq_register(IRQN_DVP_INTERRUPT, dvp_irq, NULL);   //Register external interrupt function
    plic_irq_enable(IRQN_DVP_INTERRUPT);                    //Enable external interrupt
    /* init model */
    //Load kmodel, need to be used with nncase
    ...
    while (1)
    {
        ...
        float *features;
        size_t output_size;
        // Get the KPU final processing result KPU task handle Result index value Result size (bytes)
